...
export default function ComponentsPage({ params }: Readonly<ComponentsPageProps>) {
  return (
      ...
      <div className="px-4 space-y-6 w-full">
        <div className="">
          ...
          <p className="text-sm text text-muted-foreground">
            Navigate to all the {params.slug} components available in the registry.
          </p>
        </div>
          ...
          {Object.entries(Index).map(([_, value]) => {
            return Object.entries(value).map(
              ([key, newValue]: [key: string, newValue: any]) => {
                ...
                return newValue.type === `components:${params.slug}:demo` ? (
